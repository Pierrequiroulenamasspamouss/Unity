    ...
    public class EventManager : MonoBehaviour
    {
        ...
        static void Update()
        {
            if (firstUpdate)
            {
                ...
                if (!isValid)
                {
                    // Downgraded to Warning because decompiled projects do not have the native .fspro file anyway.
                    Debug.LogWarning("FMOD Studio (Ignorable Warning): " + validateMessage);
                }
                firstUpdate = false;
